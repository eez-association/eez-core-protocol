...
// L2 -> L1, X2 scenario: read -> read -> write -> read -> read.
...
// L1: ONE immediate zero-hash L2Tx entry runs [read, read, increment, read, read]
...
interface IStaticReadX2WriteTargetL2 {
    function counter() external view returns (uint256);
}

contract StaticReadX2WriteTargetL2 is IStaticReadX2WriteTargetL2 {
...
contract StaticReadX2WriteReaderL2 {
    uint256 private constant PROXY_CALL_GAS = 5_000_000;
    StaticReadX2WriteTargetL2 public immutable target;
    uint256 public beforeRead;
    uint256 public incrementResult;
    uint256 public afterRead;
    uint256 public runs;

    constructor(StaticReadX2WriteTargetL2 remoteProxy) {
        target = remoteProxy;
    }
...
abstract contract StaticReadX2WriteL2Actions {
    ...
    function _data(bool isStatic) internal pure returns (bytes memory) {
        return isStatic
            ? abi.encodeCall(IStaticReadX2WriteTargetL2.counter, ())
            : abi.encodeCall(StaticReadX2WriteTargetL2.increment, ());
    }
    ...
    function _l1Calls(address counter, address reader) internal pure returns (L2ToL1Call[] memory calls) {
        // One L2 user transaction maps to ONE L1 entry, including all four real reads.
        calls = new L2ToL1Call[](5);
    ...
    function _l1Steps(address counter, address reader) internal pure returns (HashStep[][] memory steps) {
        ...
        steps[0] = new HashStep[](10);
        for (uint256 i; i < 5; i++) {
            steps[0][2 * i] = RollingHashBuilder.stepCallBegin(_key(i != 2, counter, reader));
            steps[0][2 * i + 1] = RollingHashBuilder.stepCallEnd(true, abi.encode(uint256(i < 2 ? 0 : 1)));
        }
    }
        ...
        returns (StaticExecutionEntryL2[] memory entries)
    {
        ...
        // Same key, future cursor FIRST. Repeated reads must not advance the cursor.
        for (uint256 i; i < 2; i++) {
...
contract Deploy is Script {
    function run() external {
        vm.startBroadcast();
        StaticReadX2WriteTargetL2 counter = new StaticReadX2WriteTargetL2();
...
contract DeployL2 is Script {
    function run() external {
        ...
        StaticReadX2WriteReaderL2 reader = new StaticReadX2WriteReaderL2(StaticReadX2WriteTargetL2(proxy));
        output("READER_L2", address(reader));
        vm.stopBroadcast();
    }
}

contract ExecuteL2 is Script, StaticReadX2WriteL2Actions {
    function run() external {
        address counter = vm.envAddress("COUNTER_L1");
        StaticReadX2WriteReaderL2 reader = StaticReadX2WriteReaderL2(vm.envAddress("READER_L2"));
        ...
        vm.stopBroadcast();
        console.log("done: L2 counter reads [0,0], increment 1, reads [1,1]");
    }
}

contract Execute is Script, StaticReadX2WriteL2Actions {
    function run() external {
        StaticReadX2WriteTargetL2 counter = StaticReadX2WriteTargetL2(vm.envAddress("COUNTER_L1"));
        ...
        vm.stopBroadcast();
        console.log("done: L1 executed real reads [0,0], increment 1, reads [1,1]");
    }
}
...
contract ExecuteNetworkL2 is Script {
    function run() external view {
        ...
        console.log("VALUE=0");
        console.log("CALLDATA=%s", vm.toString(abi.encodeCall(StaticReadX2WriteReaderL2.run, ())));
    }
}

contract VerifyNetwork is Script {
    function run() external view {
        require(StaticReadX2WriteTargetL2(vm.envAddress("COUNTER_L1")).counter() == 1, "destination counter mismatch");
...
contract VerifyNetworkL2 is Script {
    function run() external view {
        StaticReadX2WriteReaderL2(vm.envAddress("READER_L2")).assertFirstRun();
        console.log("VERIFY_PASS L2 reads [0,0] -> increment 1 -> reads [1,1], one trigger");
    }
}

contract ComputeExpected is ComputeExpectedBase, StaticReadX2WriteL2Actions {
    function run() external view {
        address counter = vm.envAddress("COUNTER_L1");
        address reader = vm.envAddress("READER_L2");
