...
| `src/interfaces/IEEZ.sol` | Shared `ProxyInfo` + `IEEZ` interface, plus the L1 execution structs (`RootUpdate`, `ExecutionEntry`, `StaticExecutionEntry`, `L2ToL1Call`, `ExpectedL1ToL2Call`, the batch structs, …) |
| `src/interfaces/IEEZL2.sol` | L2 execution structs with self-relative directional names (`CrossChainCall`, `ExpectedOutgoingCrossChainCall`, `ExecutionEntry`, `StaticExecutionEntry`) — leaner than L1's (no `RootUpdate` / `destinationRollupId` / `ExpectedRootPerRollup`) |
...
- `entryHashes[i] = keccak256(abi.encode(batch.entries[i]))` — binds the FULL `ExecutionEntry`
  struct (rootUpdates, proxyEntryHash, l2ToL1Calls, expectedL1ToL2Calls, rollingHash,
...
- `executeCrossChainCall(...)`: consumer's destination rollupId = `proxyInfo.originalRollupId`.
  ...
  preconditions (every `RootUpdate.currentRoot` == the live root); non-matches are skipped,
...
per-rollup queues unconditionally. Soundness backstop: each entry's `RootUpdate.currentRoot`
...
3. **Structural validation** (no external calls) via `_validateBatchStructure(batch)`: sorted
   ...
   `proofSystemIndexes[]` strictly ascending and in range; per entry: ≥1 `rootUpdates`
...
| "Drained cleanly" gate before publishing the remainder | Removed — `_saveRemainderEntries` runs **unconditionally** (even if the transient prefix wasn't fully drained). `RootUpdate.currentRoot` is the soundness backstop for the persistent path. |
...
- **`_processNCalls` runs before `_applyRootUpdates`**: outer entry's state deltas applied
