...
import {Counter, CounterAndProxy, PROXY_CALL_GAS} from "./mocks/CounterContracts.sol";
...
///         The explicit gas keeps the L2 gas-folded call hash reproducible by a probe.
contract OutgoingForwarderL2 {
    function fire(address proxy, bytes calldata data) external returns (uint256) {
        (bool ok,) = proxy.call{gas: PROXY_CALL_GAS}(data);
...
contract EEZL2CoverageTest is BaseL2 {
    ...
    function test_ExecuteCrossChainCall_ForwardsValueToSystem() public {
        ...
        uint256 sentValue = 5;
        // Balance read first: it warms SYSTEM_ADDRESS in the test frame, so the probe and the real
        // call observe identical warmth on the pre-capture value-forwarding path.
        uint256 sysBefore = SYSTEM_ADDRESS.balance;
        uint64 callGas = _probeOutgoing(address(this), proxy, sentValue, callData);
        // Inbound call: source = (this caller, this L2), target = (target, remote proxy rollup).
        bytes32 crossChainCallHash =
            _outgoingCallHash(address(this), address(target), REMOTE_ROLLUP_ID, sentValue, callGas, callData);
        ...
        bytes32 callHash =
            _ccHash(NOT_STATIC_CALL, address(this), REMOTE_ROLLUP_ID, address(target), TEST_ROLLUP_ID, 0, callData);
        ...
        (bool success,) = proxy.call{value: sentValue, gas: CALL_GAS}(callData);
        assertTrue(success);
    ...
    function test_ExecuteCrossChainCall_EtherTransferFailed() public {
        ...
        bytes32 h =
            _ccHash(NOT_STATIC_CALL, address(this), TEST_ROLLUP_ID, address(target), REMOTE_ROLLUP_ID, 3, callData);
        bytes32 callHash =
            _ccHash(NOT_STATIC_CALL, address(this), REMOTE_ROLLUP_ID, address(target), TEST_ROLLUP_ID, 0, callData);
        CrossChainCall[] memory calls = new CrossChainCall[](1);
        ...
        mgr2.loadExecutionTable(entries, new StaticExecutionEntryL2[](0));
    ...
    function test_IncomingCrossChainCall_EmptyEntries() public {
        ...
        manager.executeIncomingCrossChainCall(new ExecutionEntry[](0), new StaticExecutionEntryL2[](0));
    }

    function test_IncomingCrossChainCall_EmptyIncomingCalls() public {
        ExecutionEntry[] memory entries = new ExecutionEntry[](1);
        entries[0].proxyEntryHash = bytes32(uint256(1));
        vm.prank(SYSTEM_ADDRESS);
        vm.expectRevert(EEZL2.EmptyIncomingCalls.selector);
    ...
    function test_IncomingCrossChainCall_EntryHashMismatch() public {
        // incomingCalls[0]'s hash won't equal entries[0].proxyEntryHash.
        bytes memory data = abi.encodeCall(ViewTargetL2.setValue, (7));
        ExecutionEntry[] memory entries = new ExecutionEntry[](1);
        entries[0].proxyEntryHash = bytes32(uint256(0xBAD));
        CrossChainCall[] memory calls = new CrossChainCall[](1);
        calls[0] = _cc(address(target), 0, data, address(0xABCD), REMOTE_ROLLUP_ID);
    ...
    function test_IncomingCrossChainCall_Success() public {
        ...
        bytes32 inboundHash =
            _ccHash(NOT_STATIC_CALL, sourceAddr, sourceRollup, address(target), TEST_ROLLUP_ID, value, data);

        CrossChainCall[] memory calls = new CrossChainCall[](1);
        calls[0] = _cc(address(target), value, data, sourceAddr, sourceRollup);
        ...
        vm.prank(SYSTEM_ADDRESS);
        bytes memory ret = manager.executeIncomingCrossChainCall{value: value}(entries, new StaticExecutionEntryL2[](0));
    ...
    function test_ExpectedOutgoingCall_SuccessPath() public {
        ...
        bytes32 outerCallHash =
            _ccHash(NOT_STATIC_CALL, address(this), REMOTE_ROLLUP_ID, address(cap), TEST_ROLLUP_ID, 0, outerCd);
        ...
        bytes32 subHash = _ccHash(NOT_STATIC_CALL, address(cap), MAINNET, counterL1, TEST_ROLLUP_ID, 0, innerCd);
        ...
        bytes32 h = _hEntryBeginL2(outerHash);
    ...
    /// Top-level lookup carrying a real static sub-call: `_processStaticIncomingCalls` runs it
    ...
    /// A static sub-call carrying a `revertNextNCalls` span reverts StaticCallWithRevertSpan —
    ...
    function _nestedStaticLookup(bool retry, bool failAtOne) internal {
        ...
        bytes32 outerCallHash =
            _ccHash(NOT_STATIC_CALL, address(0xD00D), REMOTE_ROLLUP_ID, address(reader), TEST_ROLLUP_ID, 0, outerData);
        ...
        (bool ok,) = outerProxy.call{gas: CALL_GAS}(outerData);
    ...
    function test_NestedReentrant_NoMatch_CallNotFound() public {
        ...
        bytes32 outerCallHash =
            _ccHash(NOT_STATIC_CALL, address(this), REMOTE_ROLLUP_ID, address(fwd), TEST_ROLLUP_ID, 0, outerData);
        // Reentrant call leaving this L2: source forced to ROLLUP_ID, target (innerRemote, MAINNET).
        bytes32 innerHash = _outgoingCallHash(address(fwd), innerRemote, MAINNET, 0, innerGas, innerData);
        ...
        (bool ok,) = outerProxy.call{gas: OUTER_CALL_GAS}(outerData);
    ...
    function test_IncomingStaticCall_InsideEntry() public {
        ...
        bytes32 proxyEntryHash = _outgoingCallHash(address(this), address(target), REMOTE_ROLLUP_ID, 0, callGas, cd);
        bytes32 staticCallHash =
            _ccHash(IS_STATIC, address(this), REMOTE_ROLLUP_ID, address(target), TEST_ROLLUP_ID, 0, cd);
        ...
        bytes32 h =
            _hCallEnd(_hCallBegin(_hEntryBeginL2(proxyEntryHash), staticCallHash), true, abi.encode(uint256(55)));
        ...
        (bool ok,) = proxy.call{gas: CALL_GAS}(cd);
