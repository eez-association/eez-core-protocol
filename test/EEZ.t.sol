...
contract EEZTest is Base {
        ...
        returns (ExecutionEntry memory e)
    {
        ...
        e.rollingHash = _hEntryBegin(e.stateDeltas, proxyEntryHash);
        e.success = false;
    ...
    function test_PostBatch_MultipleEEZ_OneEntryEach() public {
        ...
        deltas[0] =
            StateDelta({rollupId: uint64(r1.id), currentState: bytes32(0), newState: keccak256("s1"), etherDelta: 0});
        deltas[1] =
            StateDelta({rollupId: uint64(r2.id), currentState: bytes32(0), newState: keccak256("s2"), etherDelta: 0});
    ...
    function test_PostBatch_DifferentBlocks_LazyReset() public {
        ...
        e1[0].rollingHash = _hEntryBegin(e1[0].stateDeltas, ah);
    ...
    function test_ExecuteCrossChainCall_Simple() public {
        ...
        StateDelta[] memory deltas = _oneDelta(rid, bytes32(0), keccak256("after"), 0);
    ...
    function test_PostBatch_EtherDeltasNonZeroSum_ImmediateSkipped() public {
        ...
        StateDelta[] memory deltas = _oneDelta(r.id, bytes32(0), keccak256("s1"), 1 ether);
        ExecutionEntry[] memory entries = new ExecutionEntry[](1);
        ...
        returns (ExecutionEntry[] memory entries, ValueForwarder forwarder)
    {
        ...
        StateDelta[] memory deltas = _oneDelta(rid, bytes32(0), keccak256("s1"), etherDelta);
    ...
    function test_UnconsumedCalls_Reverts() public {
        ...
        calls[1] = calls[0];
        StateDelta[] memory deltas = _oneDelta(rid, bytes32(0), keccak256("s"), 0);
        ...
        returns (ExecutionEntry memory e)
    {
        ...
        StateDelta[] memory deltas = _oneDelta(rid, _getRollupState(rid), keccak256("rev"), 0);
    ...
    function test_NestedRevertedLookup_EntryScoped_RevertsAndCatches() public {
        ...
        StateDelta[] memory deltas = _oneDelta(rid, bytes32(0), keccak256("s1"), 0);
